...
image_to_tf::image_to_tf():
  pnh("~"),it_(nh_),tfListener(tfBuffer),file_numbering(0)
{
...
void image_to_tf::imageCb(const sensor_msgs::CameraInfoConstPtr& camera_msg,
                         const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    if (cv_ptr->image.rows > 60 && cv_ptr->image.cols > 60)
      cv::circle(cv_ptr->image, cv::Point(col_px, row_px), 10, CV_RGB(255,0,0));
    // Update GUI Window
    // Output modified video stream
    image_pub_.publish(cv_ptr->toImageMsg());
    image_to_tf::set_line(cv::Point(col_px, row_px),camera_msg);
    ...
    point_msg2.y=0;
    point_msg2.z=0;
    plane_.cells.push_back(point_msg2);

    geometry_msgs::Point point_msg3;
    point_msg3.x=1;
    point_msg3.y=1;
    point_msg3.z=0;
    plane_.cells.push_back(point_msg3);

    geometry_msgs::Point point_msg4;
    point_msg4.x=0;
    point_msg4.y=1;
    point_msg4.z=0;
    plane_.cells.push_back(point_msg4);*/
...
void image_to_tf::get_line_high(const geometry_msgs::Point& p_P,const geometry_msgs::Point& p_C )
  {   
